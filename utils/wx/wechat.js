...
wechat.api = {
  ...
  message: {
    ...
    receiveHandler: {
      ...
      subscribeHandle: () => {
        wechat.api.message.replyer.replyTextMsg( { toUserName: wechat.storage.reqMsg.FromUserName, content: 'Hi~ 归谷的童鞋,你好!!' } );
      },
      unsubscribeHandle: () => {
      ...
      textHandle: async () => {
        ...
        if ( content === '0' ) {
          ...
          wechat.api.message.replyer.replyImgMsg( { toUserName: wechat.storage.reqMsg.FromUserName, mediaId: result.media_id } );
        }
        else if ( content === '2' ) {
          ...
          wechat.api.message.replyer.replyTextMsg( { toUserName: wechat.storage.reqMsg.FromUserName, content: '伍' } );
        }
        else if ( content === '6' ) {
          wechat.api.message.replyer.replyTextMsg( { toUserName: wechat.storage.reqMsg.FromUserName, content: '陆' } );
        }
        else if ( content === '7' ) {
          wechat.api.message.replyer.replyTextMsg( { toUserName: wechat.storage.reqMsg.FromUserName, content: '柒' } );
        }
        else if ( content === '8' ) {
          wechat.api.message.replyer.replyTextMsg( { toUserName: wechat.storage.reqMsg.FromUserName, content: '捌' } );
        }
        else if ( content === '9' ) {
          wechat.api.message.replyer.replyTextMsg( { toUserName: wechat.storage.reqMsg.FromUserName, content: '玖' } );
        }
        else {
          wechat.api.message.replyer.replyTextMsg( { toUserName: wechat.storage.reqMsg.FromUserName, content: '文本消息。' } );
        }
      },
      imgHandle: () => {
        wechat.api.message.replyer.replyTextMsg( { toUserName: wechat.storage.reqMsg.FromUserName, content: '图片消息。' } );
      },
      voiceHandle: () => {
        wechat.api.message.replyer.replyTextMsg( { toUserName: wechat.storage.reqMsg.FromUserName, content: '语音消息。' } );
      },
      videoHandle: () => {
        wechat.api.message.replyer.replyTextMsg( { toUserName: wechat.storage.reqMsg.FromUserName, content: '视频消息。' } );
      },
      shortVideoHandle: () => {
        wechat.api.message.replyer.replyTextMsg( { toUserName: wechat.storage.reqMsg.FromUserName, content: '小视频消息。' } );
      },
      locationHandle: () => {
        wechat.api.message.replyer.replyTextMsg( { toUserName: wechat.storage.reqMsg.FromUserName, content: '地理位置消息。' } );
      },
      linkHandle: () => {
        wechat.api.message.replyer.replyTextMsg( { toUserName: wechat.storage.reqMsg.FromUserName, content: '链接消息。' } );
      },
    },
    replyer: {
